locals {
  ...
  vm_subnet_cidr = cidrsubnet(local.vnet_address_space, 7, 40)
...
# Assign Key Vault Secrets Officer scoped to the Key Vault Resource Group
...
module "vnet" {
  ...
  private_subnet_prefixes  = [local.vm_subnet_cidr]
...
module "nat_gateway" {
  ...
  vnet_vm_subnet_id   = module.vnet.vm_subnet_id
...
module "keyvault" {
  ...
  vault_name          = "kv-hub-prod-unique12345"
...
# 3. Create the Federated Identity Credential (The "Trust Link")

# Azure needs to know who is allowed to access the "Petclinic Identity".
# This resource tells Azure: "If a pod in the staging namespace of my AKS cluster is using the petclinic-sa service account, trust it, and hand it the tokens for this identity."
resource "azurerm_federated_identity_credential" "petclinic_fic" {
  ...
  subject             = "system:serviceaccount:staging:petclinic-sa" 
}