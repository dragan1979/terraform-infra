locals {
  ...
  # 32 - 22 = 10, and 2¹⁰ = 1,024 IPs. So we have 1,024 IPs to carve up among our subnets.

  # netnum = (Third Octet x Row Multiplier) + Fourth Octet/Chunk Size
  ...
  # Target Mask: /26 
  # Row Multiplier is (26-22) 4
  # Chunk Size is 64 (32 - 26) = 2^6 = (2 x 2 x 2 x 2 x 2 x 2) = 64
  # Target IP: Third Octet is 1, Fourth Octet is 0.The Math: (1 x 4) + (0 / 64) = 4 + 0 =4
  ...
  # postgres Subnet: 10.0.1.80/28 
  # Offset: 336 IPs. 336 / 16 = 21. 
  postgres_subnet_cidr = cidrsubnet(local.vnet_address_space, 6, 21) # Changed from 25 to 21

  # Private Endpoint Subnet: 10.0.1.96/28 
  # Offset: 352 IPs. 352 / 16 = 22.
  pe_subnet_cidr = cidrsubnet(local.vnet_address_space, 6, 22) # Changed from 26 to 22

  # Public Jumpbox Subnet: 10.0.1.112/28 
  # Offset: 368 IPs. 368 / 16 = 23.
  public_jumpbox_cidr = cidrsubnet(local.vnet_address_space, 6, 23) # Changed from 28 to 23

  # Pipeline Agent Subnet: 10.0.1.128/29 (8 IPs. Uses 10.0.1.128 to 10.0.1.135)
  # Formula: 29 - 22 = 7 newbits. Grab the 48th block.
  pipeline_agent_cidr = cidrsubnet(local.vnet_address_space, 7, 48)
...
module "vnet" {
  ...
  pipeline_agent_subnet_prefixes = [local.pipeline_agent_cidr]
...
module "nat_gateway" {
  ...
  vnet_runner_subnet_id = module.vnet.pipeline_agent_subnet_id
...
module "keyvault" {
  ...
  # Pass current Object ID and Tenant ID to the module
...
#   subnet_id           = module.vnet.pipeline_agent_subnet_id
#   vm_size             = "Standard_B2s_v2"
...
module "postgres" {
  source              = "../../../modules/database-hardware"
...
resource "azurerm_user_assigned_identity" "petclinic_identity" {
  name                = "id-petclinic-staging"
  location            = azurerm_resource_group.aks_rg.location
  resource_group_name = azurerm_resource_group.aks_rg.name
  tags                = local.common_tags
}

# Grant the Identity read access to Key Vault secrets (The "Permission")
